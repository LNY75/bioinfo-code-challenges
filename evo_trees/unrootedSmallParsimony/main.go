...
func ReadInput(input string) (int, []*Node) {
	...
	i := len(lines) - 2
	adj := strings.Split(lines[i], "->")
	n1ID, err := strconv.Atoi(adj[0])
	n2ID, err := strconv.Atoi(adj[1])
	rootID := n2ID + 1
	n1 := T[n1ID]
	n2 := T[n2ID]
	if err != nil {
	...
	root := &Node{id: rootID, child1: n1, child2: n2}
	n1.parent = root
	n2.parent = root
...
func AssignLenToParent(n int, root *Node, internalSeqs []string, T []*Node) []*Node {
	...
	len := DifferenceScore(n1.seq, n2.seq)
	n1.lenToParent = len
	n2.lenToParent = len
